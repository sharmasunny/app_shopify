...
for($i=1;$i<=$page;$i++){	
	...
	foreach($orgorder as $res) {
		...
		$order_res_all = json_encode($res);
		$order_number = $res['id'];


		/*
		*	@this query will insert a record in orders table
		*	@field order_id
		*	@field order_price
		...
		*/
		...
		$add_order = "INSERT INTO ".$orders_table."(order_id,order_number,order_price,customer_email,order_shipping_address,	all_record,order_driver_id) VALUES ('".$order_id."','".$order_number."',".$order_price.", '".$customer_email."','".$order_shipping_address."','".$order_res_all."','') ON DUPLICATE KEY UPDATE order_price = VALUES(order_price),customer_email = VALUES(customer_email),order_shipping_address = VALUES(order_shipping_address)";
				...
				<?php
				 while($users_record = $users_records->fetch_assoc()) {
					...
					if($driver_id == "0") {
						...
						/*
						*	@this query will select driver name from drivers where driver id match 
						*	@field driver_id
						*/
					...
					<tr>
						...
						<td><?php if(!empty($driver_name)) { echo $driver_name; } else { echo 'No driver selected!'; } ?><br><a href="inc/order-management/get_change_log.php?oid=<?php echo $orderid; ?>" class="iframe">Change log</a></td>          
