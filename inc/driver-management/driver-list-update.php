...
if ( !empty( $info ) ) {
	...
	if ( $info=="enable" ) {
		...
		if ( !empty( $enbid ) ) {
			...
			*	@field driver_id 
			*/
			$sql = "update ".$drivers_table." set driver_status='1' where driver_id=".$enbid." ";
			...
			if ($conn->query($sql) === TRUE) {
			   ...
			   echo "<script>window.location = '".HOME_URL."'</script>";
				...
				echo "<script>alert('Error while Update Record !!');</script>";
				 echo "<script>window.location = '".HOME_URL."'</script>";
				//echo "<div class='clearb'></div><div class='updated' id='message'><p><strong>Error deleting record.".$conn->error."</strong></p></div>";
			}
	...
	if ( $info=="disable" ) {
		...
		if ( !empty( $enbid ) ) {
			...
			*	@field driver_id 
			*/
			$sql = "update ".$drivers_table." set driver_status='0' where driver_id=".$enbid." ";
